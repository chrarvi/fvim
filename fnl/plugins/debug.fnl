...
(fn project-python []
  (let [root (vim.fs.root 0 :.git)
        marker (and root (vim.fs.joinpath root :confs :coding :ty.toml))
        python (and root (vim.fs.joinpath root :tools :venv :bin :python))]
    (if (and marker (= (vim.fn.filereadable marker) 1) python
...
(fn project-dap-config [config]
  (let [root (vim.fs.root 0 :.git)
        wrapper (and root (vim.fs.joinpath root :tools :bin :uv_run.sh))]
    (if (and (= config.request :launch) wrapper
             (= (vim.fn.executable wrapper) 1))
        (vim.tbl_extend :force config {:cwd root :python [wrapper :python]})
        config)))
...
(tset dap.listeners.on_config :project-uv project-dap-config)
...
(vim.api.nvim_create_user_command :RunScriptWithArgs
                                  (fn [t]
                                    (let [args (vim.split (vim.fn.expand t.args)
                                                          "\n")
                                          approval (vim.fn.confirm (.. "Will try to run:
    "
                                                                       vim.bo.filetype
                                                                       " "
                                                                       (vim.fn.expand "%")
...
(map! [n] :<leader>dR ":RunScriptWithArgs " "Run script with arguments")
